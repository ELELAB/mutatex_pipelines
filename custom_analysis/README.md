...
The underlying assumption for using the pipeline is that the stability\_scan pipeline has been run and the same filetree is produced for {coding\_gene}/stability/mutatex\_runs and {coding_gene}/stability/mutatex\_input\_preparation.
The pipeline will automatically create a soft link to the input files contained in {coding\_gene}/stability/mutatex\_runs/{database}\_{range}/model\_{version}/saturation inside the folder {coding\_gene}/stability/mutatex\_analyses/{database}\_{range}/model\_{version}/saturation in order to run the analysis.
